...
- **Segments** — current directory, git branch + dirty marker + ahead/behind
  vs upstream, Node.js version, active Python env (venv/conda), last-command
  duration (when slow).
...
- **Color-aware** — honors `NO_COLOR` (emits no color at all) and degrades hex
  to the nearest of the 16 ANSI colors on terminals without truecolor.
